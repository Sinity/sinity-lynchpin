...
| `reddit` | Comment corpus + GDPR exports | `/realm/data/reddit/reddit_comments.csv`, `/realm/data/reddit/gdpr/raw/<date>/` |
...
| `spotify` | Account Data + Extended Streaming JSON | `/realm/data/spotify/**` |
...
| `webhistory` | Gestalt merged history | `/realm/data/webhistory/gestalt/data/**` |
...
All modules cache into `artefacts/lynchpin/cache/` and register tables when `python -m lynchpin.warehouse` runs.
...
- **Views:** `pipelines/focus/calendar/` becomes a “view layer” that:
...
- **Velocity / git overlays:** Continue running `pipelines/meta/velocity/plot_velocity.py` before each refresh so the velocity HTML is mirrored into the calendar site. Consider embedding tokei stats and repo bundle summaries per day/week.
...
- **Sinnix:** treat Lynchpin as its data source for dashboards. Implement `lynchpin.sinnix` soon so Sinnix CLIs/skills can read instrumentation configs + statuses without shelling out.
...
2. **Module implementation**
   - Harden `lynchpin.sinnix`/`lynchpin.meta` and build the remaining `lynchpin.sinex` scaffolding.
   - Prototype chat webapp scraping module (even if it just shells out to an existing Chrome profile + Playwright at first).
3. **Calendar refactor**
   - Rework `pipelines/focus/calendar` to query modules/DuckDB.
...
4. **Warehouse & Datasette**
   - Expand `lynchpin.warehouse` ingest to cover Reddit, Spotify, finance, Polylogue, instrument metadata, etc., and publish a metadata JSON for Datasette.
