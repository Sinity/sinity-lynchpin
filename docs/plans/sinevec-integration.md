...
## Collections
- `sessions-v1` – Level-1 progressive summaries backed by `config/knowledge/sessions/` and `artefacts/knowledge/sessions/`.
...
2. For each artefact type, emit an NDJSON payload with fields: `id`, `text`, `metadata`, `channel`, `category`, using direct `lynchpin.sources.*`, `lynchpin.analysis.*`, or ledger/session-summary outputs rather than reviving removed warehouse surfaces.
...
## Automation Hooks
- If a dedicated exporter is needed, keep it under `lynchpin.analysis` or `lynchpin.scripts`, not under `pipelines/`.
...
- [ ] Backfill existing session summaries and other text artifacts once the export shape is stable.
