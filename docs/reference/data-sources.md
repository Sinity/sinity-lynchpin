...
consumers. It intentionally does not try to enumerate every inherited wrapper
entrypoint, especially older `life_timeline` commands that are slated for
absorption into the unified system.
...
| Google Takeout | Archive bundles | Google | `/realm/data/exports/google/raw/takeout/*.tgz` (plus `*.zip`) | Download via https://takeout.google.com; keep all parts together in raw. | `lynchpin.sources.exports.takeout`, `my.google.takeout.parser`, long-range retrospective evidence gathering |
