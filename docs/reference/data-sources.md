...
| Messenger (API export) | fbmessengerexport SQLite | fbmessengerexport | `/realm/data/exports/comms/facebook-messenger/processed/fbmessengerexport.sqlite` | Refresh via `python -m lynchpin.ingest.fbmessenger_export` (reads Chrome cookies; direct decrypt → headless Chrome remote-debugging fallback). Use `--cookies`/`--cookies-file` or `--cookie-db` if the default cookies DB isn’t usable. | `my.fbmessenger.export` |
