...
_LINE_CLASS_PATTERNS: list[tuple[str, tuple[str, ...]]] = [
    ("import_or_dep", ("import ", "from ", "use ", "extern crate", "require ", "#include", "#include")),
    ("fn_sig_or_def", ("def ", "class ", "async def", "fn ", "struct ", "enum ", "trait ", "impl ")),
    ("test_assert", ("assert", "test ", "it(", "describe(", "test!(", "#[test]", "#[cfg(test)]")),
    ("schema_or_field", (": str", ": int", ": float", ": bool", ": dict", ": list", "Column(", "Field(")),
    ("config_or_option", ("config", "settings", "options", "env", "CLI argument", "argparse")),
    ("error_or_log", ("raise ", "except ", "log.", "logger.", "error!", "warn!", "info!", "debug!")),
    ("storage_or_query", ("execute(", "SELECT ", "INSERT ", "UPDATE ", "DELETE ", "commit(", "save(")),
    ("docs_or_prose", ("#", "//", "/*", "*/", "\"\"\"", '"""', "## ", "> ")),
]
    ...
    return {
        ...
        "summary": {
            ...
            "top_commits_by_symbol_count": sorted(
                ({"sha": r["short_sha"], "subject": r["subject"], "symbol_count": r["symbol_count"]}
...
def _extract_symbols(
    repo_path: str, sha: str, files: list[dict[str, Any]], row: dict[str, Any],
) -> list[dict[str, Any]]:
...
def _classify_operations(
    files: list[dict[str, Any]], symbols: list[dict[str, Any]],
) -> dict[str, float]:
    ops: dict[str, float] = {}
    paths = [f.get("path", "") for f in files]
    if any(p and ("test" in p.lower() or "spec" in p.lower() or "conftest" in p.lower()) for p in paths):
        ops["test_or_verification"] = max(ops.get("test_or_verification", 0), 0.8)
    if any(p and ("cli" in p.lower() or "main.py" == p or "main.rs" == p or "argparse" in p.lower()) for p in paths):
        ops["cli_or_user_surface"] = max(ops.get("cli_or_user_surface", 0), 0.7)
    if any(p and ("schema" in p.lower() or "model" in p.lower() or "migration" in p.lower()) for p in paths):
        ops["data_model_or_schema"] = max(ops.get("data_model_or_schema", 0), 0.7)
    if any(p and ("storage" in p.lower() or "db" in p.lower() or "repository" in p.lower() or "database" in p.lower()) for p in paths):
        ops["persistence_or_materialization"] = max(ops.get("persistence_or_materialization", 0), 0.7)
    if any(p and ("config" in p.lower() or "settings" in p.lower() or "nix" in p.lower() or ".toml" in p or ".nix" in p) for p in paths):
        ops["config_or_deployment"] = max(ops.get("config_or_deployment", 0), 0.7)
    if any(p and ("doc" in p.lower() or "readme" in p.lower() or "changelog" in p.lower() or ".md" in p.lower()) for p in paths):
        ops["documentation_or_narrative"] = max(ops.get("documentation_or_narrative", 0), 0.8)
    if any(p and ("ci" in p.lower() or "lint" in p.lower() or "mypy" in p.lower() or "type" in p.lower()) for p in paths):
        ops["type_or_lint_guardrail"] = max(ops.get("type_or_lint_guardrail", 0), 0.7)
    if any(p and ("error" in p.lower() or "exception" in p.lower() or "log" in p.lower()) for p in paths):
        ops["error_handling_or_diagnostics"] = max(ops.get("error_handling_or_diagnostics", 0), 0.6)
    if any(s["kind"] in ("function", "class") for s in symbols):
        ops["internal_behavior"] = max(ops.get("internal_behavior", 0), 0.5)
        if any("api" in s.get("name", "").lower() or "public" in s.get("name", "").lower() for s in symbols):
    ...
    if any("refactor" in p.lower() or "rename" in p.lower() or "move" in p.lower() for p in paths):
        ops["refactor_move_or_extract"] = max(ops.get("refactor_move_or_extract", 0), 0.6)
...
def _impact_assessment(
    files: list[dict[str, Any]], symbols: list[dict[str, Any]], operations: dict[str, float],
) -> dict[str, str]:
    impact: dict[str, str] = {}
    paths = [f.get("path", "") for f in files]
    if any("api" in p.lower() or "public" in p.lower() or "__init__" in p for p in paths):
    ...
    impact["runtime_behavior"] = "likely" if operations.get("internal_behavior", 0) > 0.4 else "possible"
    impact["data_contract"] = "likely" if operations.get("data_model_or_schema", 0) > 0.5 else "none"
    impact["guardrail"] = "likely" if operations.get("test_or_verification", 0) > 0.5 else "possible"
    return impact


def _risk_flags(
    files: list[dict[str, Any]], symbols: list[dict[str, Any]], row: dict[str, Any],
) -> list[str]:
    flags: list[str] = []
    paths = [f.get("path", "") for f in files]
    ...
    if any("schema" in p.lower() or "migration" in p.lower() or "storage" in p.lower() for p in paths):
        if not any("test" in p.lower() for p in paths) and categories.get("test", 0) == 0:
            flags.append("schema_or_storage_change_without_test")
    if any("security" in p.lower() or "auth" in p.lower() or "secret" in p.lower() for p in paths):
...
def _semantic_caveats(
    ...
    caveats: list[str] = []
    non_py = [f for f in files if f.get("path", "") and not f.get("path", "").endswith(".py")]
    if non_py:
        caveats.append(f"{len(non_py)} non-Python files — symbol extraction is Python-only")
    ...
    if not files:
        caveats.append("no file diff data available — semantic classification is path-based")
    return caveats
...
def _files_from_row(row: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if isinstance(paths, list):
        return [{"path": str(p), "status": "modified", "hunk_count": 0, "added_lines": 0, "deleted_lines": 0} for p in paths[:30]]
    return []
