...
def _package_prefix() -> tuple[str, ...]:
    if _LYNCHPIN_ROOT.name == "analysis":
        return ("lynchpin", "analysis")
    return (_LYNCHPIN_ROOT.name,)


def _target_subpackage(parts: Iterable[str]) -> str | None:
    ...
    return target if target in set(_subpackages()) else None


def _resolve_import_from(py_file: Path, node: ast.ImportFrom) -> str | None:
    prefix = _package_prefix()
    if node.level == 0:
        return _target_subpackage((node.module or "").split("."))

    rel = py_file.relative_to(_LYNCHPIN_ROOT).with_suffix("")
    package_parts = prefix + rel.parts[:-1]
    keep = len(package_parts) - node.level + 1
    if keep < len(prefix):
        return None
    module_parts = tuple((node.module or "").split(".")) if node.module else ()
    return _target_subpackage(package_parts[:keep] + module_parts)
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for alias in node.names:
                target = _target_subpackage(alias.name.split("."))
                if target:
                    imports.add(target)
        elif isinstance(node, ast.ImportFrom):
            target = _resolve_import_from(py_file, node)
            if target:
                imports.add(target)
    return imports
...
def _import_graph() -> List[ImportEdge]:
    ...
    for py_file in _LYNCHPIN_ROOT.glob("*.py"):
        for target in _extract_imports(py_file):
            if target in subpackages:
                edges.add(("__root__", target))
