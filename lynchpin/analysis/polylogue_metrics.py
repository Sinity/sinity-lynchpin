...
try:
    ...
    for day in days:
        ...
        project_counter.update(day.projects_active)
...
def _bundle_context() -> dict[str, Any] | None:
    record = preferred_bundle(
        discover_bundle_records(),
        "polylogue_communication_diagnosis_bundle",
    )
    if record is None:
        return None
    summary = read_bundle_json(record, suffix="summary.json")
    subsystem_rows = read_bundle_csv_rows(record, suffix="subsystem_summary.csv")
    return {
        "bundle": record.filename,
        "summary": summary,
        "subsystem_rows": subsystem_rows[:20],
    }
