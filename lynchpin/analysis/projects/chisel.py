...
def _compressed_header(plan: RepoPlan, git: dict, generated_at: str) -> str:
    ...
    return '\n'.join((f'Project: {plan.name}', f'Source: {plan.path}', f'Slice: compressed (full repo, Tree-sitter structure extraction)', f'Generated: {generated_at}', f"Branch: {git['branch']} · Commit: {git['commit']} · Dirty: {git['dirty']}", f"Slices this summarises: {', '.join((s.name for s in plan.slices))}", 'Generated by chisel (lynchpin) via repomix.'))
...
def _run_scratchpad(repomix_bin: str, output_dir: Path, plan: RepoPlan, git: dict, generated_at: str) -> tuple[str, int] | None:
    ...
    header = '\n'.join((f'Project: {plan.name}', f'Source: {plan.path}/.agent/scratch/', f'Slice: scratchpad — working notes, debugging analysis, temporary reasoning', f'Generated: {generated_at}', f"Branch: {git['branch']} · Commit: {git['commit']} · Dirty: {git['dirty']}", 'Generated by chisel (lynchpin) via repomix.'))
...
def _build_issues_xml(issues: list[dict], repo_slug: str, state: str, generated_at: str) -> str:
    ...
    for iss in issues:
        ...
        lb.text = ', '.join((l['name'] for l in iss.get('labels', [])))
