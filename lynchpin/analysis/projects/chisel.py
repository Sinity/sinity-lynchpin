...
def _issues_from_context_product(repo_slug: str, state: str, limit: int) -> list[dict]:
    ...
    for row in iter_github_context(ensure=False):
        item = row.item
        if item.kind != 'issue' or item.slug.lower() != wanted_slug:
            continue
        if state != 'all' and item.state != state:
            continue
        issues.append(_github_issue_to_chisel_dict(item))
        if len(issues) >= limit:
            break
    return issues
...
def _generate_issues(plan: RepoPlan, out_dir: Path, generated_at: str) -> tuple[int, int]:
    ...
    open_issues = _issues_from_context_product(plan.github_slug, 'open', DEFAULT_ISSUE_LIMIT)
    _normalize_comments(open_issues)
    closed_issues = _issues_from_context_product(plan.github_slug, 'closed', DEFAULT_ISSUE_LIMIT)
