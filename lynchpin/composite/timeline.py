    ...
    for e in entries_in_range(start, end):
        ...
        for seg in e.segments:
            if seg.start and seg.end and seg.start != datetime.min:
                arch = arch_by_date.get(logical_date(seg.start))
                stages = ""
                if arch is not None:
                    stages = f", deep {arch.deep_min:.0f}m, REM {arch.rem_min:.0f}m, awake {arch.awake_min:.0f}m"
                yield TimelineEvent(
                    start=seg.start, end=seg.end, source="sleep", kind="sleep",
                    summary=f"Sleep {hours}h{quality}{stages}",
