...
class MaterializationExecutor:
    """Typed binding from a source contract to an owned materializer.

    ``materialization_hint`` remains the operator-facing description for manual
    debugging and repair. Normal read paths call materializers directly through
    ``lynchpin.materialization`` when a local product is missing or outside its
...
SOURCE_CONTRACTS: tuple[SourceContract, ...] = (
    ...
    SourceContract(
        name="xiaomi_cloud",
        authority="Sinnix Xiaomi cloud witness capture lane",
        query_surface="lynchpin.sources.xiaomi_cloud.latest_envelopes",
        materialization_hint="sinnix records this live; no Lynchpin materializer",
        required=False,
        collection_model="continuous",
    ),
    SourceContract(
    ...
    SourceContract(
        name="notifications",
        authority="sinnix-capture-v1 desktop-notification event lane",
        query_surface="lynchpin.sources.sinnix_capture_lanes.notification_events",
        materialization_hint="sinnix records this live; no Lynchpin materializer",
        required=False,
        collection_model="continuous",
    ),
    SourceContract(
        name="mpris",
        authority="sinnix-capture-v1 media-player-state event lane",
        query_surface="lynchpin.sources.sinnix_capture_lanes.mpris_events",
        materialization_hint="sinnix records this live; no Lynchpin materializer",
        required=False,
        collection_model="continuous",
    ),
    SourceContract(
        name="audio_index",
        authority="sinnix-capture-v1 speech-segment index over the audio capture",
        query_surface="lynchpin.sources.sinnix_capture_lanes.audio_index_entries",
        materialization_hint="sinnix records this live; no Lynchpin materializer",
        required=False,
        collection_model="continuous",
    ),
    SourceContract(
        name="audio_topology",
        authority="sinnix-capture-v1 PipeWire graph-topology event lane",
        query_surface="lynchpin.sources.sinnix_capture_lanes.audio_topology_events",
        materialization_hint="sinnix records this live; no Lynchpin materializer",
        required=False,
        collection_model="continuous",
    ),
    SourceContract(
        name="screen_frames",
        authority="sinnix-capture-v1 per-window screen-frame event lane",
        query_surface="lynchpin.sources.sinnix_capture_lanes.screen_frame_events",
        materialization_hint="sinnix records this live; no Lynchpin materializer",
        required=False,
        collection_model="continuous",
    ),
    SourceContract(
        name="phone_events",
        authority="Sinnix phone app estate event stream (instrument runs, marks, ambient light/motion, chunk lifecycle)",
        query_surface="lynchpin.sources.phone_events",
        materialization_hint="phone app records this live; no Lynchpin materializer",
        required=False,
        collection_model="continuous",
    ),
    SourceContract(
        name="phone_ambient",
        authority="Sinnix phone app ambient-audio loudness levels",
        query_surface="lynchpin.sources.phone_ambient",
        materialization_hint="phone app records this live; no Lynchpin materializer",
        required=False,
        collection_model="continuous",
    ),
    SourceContract(
        name="steering",
        authority="Steering store nightly JSONL export (commitments/activities/reviews)",
        query_surface="lynchpin.sources.steering",
        materialization_hint="steering ritual records this live; no Lynchpin materializer",
        required=False,
        collection_model="continuous",
    ),
    SourceContract(
        name="transcripts",
        authority="sinnix speech-transcription pipeline output over phone/desktop audio captures",
        query_surface="lynchpin.sources.transcripts",
        materialization_hint="speech-hub transcription pipeline records this live; no Lynchpin materializer",
        required=False,
        collection_model="continuous",
    ),
    SourceContract(
