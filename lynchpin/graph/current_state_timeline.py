...
def build_current_state_timeline(graph: EvidenceGraph, *, start: date, end: date) -> CurrentStateTimeline:
    ...
    for day in all_days:
        ...
        if not rows and (not signals) and (not day_chains) and (not day_closures):
            continue
        days.append(DaySection(day=day, rows=rows, chains=day_chains, closure_transitions=day_closures, signals=signals))
        total_rows += len(rows)
        total_chains += len(day_chains)
    return CurrentStateTimeline(start=start, end=end, generated_at=datetime.now(timezone.utc), days=tuple(days), total_rows=total_rows, total_chains=total_chains)
...
def render_current_state_timeline(timeline: CurrentStateTimeline) -> str:
    ...
    lines: list[str] = [f'# Current-State Timeline ({timeline.start.isoformat()} → {timeline.end.isoformat()})', '', f"_Generated {timeline.generated_at.isoformat(timespec='seconds')} • {timeline.total_rows} events • {timeline.total_chains} causal chains • {len(timeline.days)} active days_", '', '_Sibling of the current-state context pack. The pack summarizes; this artifact gives full chronological detail with node IDs and citation data so claims can be traced back to evidence._', '']
    ...
    for section in timeline.days:
        ...
        if section.signals:
            ...
            lines.append('')
        if section.chains:
            lines.append('**Causal chains fired**')
            for chain in section.chains:
                node_refs = ', '.join(chain.node_ids)
                gaps = ', '.join((f'{g:.0f}m' for g in chain.time_gaps_minutes))
                lines.append(f'- {chain.summary} (confidence {chain.confidence:.0%}, gaps {gaps}, nodes: {node_refs})')
            lines.append('')
        if section.closure_transitions:
            ...
            for closure in section.closure_transitions:
                ...
                lines.append(f"- {closure.project} {closure.issue_ref} → **{closure.closure_status}** (prs={prs}, commits={shas}{(f' — {caveat}' if caveat else '')})")
            lines.append('')
        if section.rows:
            ...
            for row in section.rows:
                ...
                lines.append(f"| {when} | {row.project or 'unattributed'} | {row.kind} | {row.source} | {_summary_cell(row.summary)} | {_citation_cell(row)} |")
            lines.append('')
    return '\n'.join(lines).rstrip()
_SIGNAL_KINDS = frozenset({'temporal_changepoint', 'temporal_trend', 'temporal_anomaly', 'temporal_rhythm', 'readiness_forecast'})

def _node_to_row(node: EvidenceNode) -> TimelineRow:
    payload = node.payload or {}
    excerpt: dict[str, Any] = {}
    if node.kind == 'commit':
        excerpt = {'commit': payload.get('commit'), 'github_refs': payload.get('github_refs'), 'files_changed': payload.get('files_changed'), 'paths_count': len(payload.get('paths') or ())}
    elif node.kind == 'ai_work_event':
        excerpt = {'event_id': payload.get('event_id'), 'kind': payload.get('kind'), 'kind_tier': payload.get('kind_tier'), 'kind_source': payload.get('kind_source'), 'duration_min': round((payload.get('duration_ms') or 0) / 60000, 1), 'files': (payload.get('file_paths') or [])[:3]}
    elif node.kind == 'ai_session':
        excerpt = {'conversation_id': payload.get('conversation_id'), 'provider': payload.get('provider'), 'messages': payload.get('message_count'), 'work_event_kind': payload.get('work_event_kind')}
    elif node.kind in {'github_issue', 'github_pr', 'github_ref'}:
        excerpt = {'number': payload.get('number'), 'state': payload.get('state'), 'lifecycle': payload.get('lifecycle')}
    elif node.kind == 'terminal_session':
        excerpt = {'cwd': payload.get('cwd'), 'command_count': payload.get('command_count'), 'error_count': payload.get('error_count'), 'top_commands': (payload.get('commands_summary') or [])[:3]}
    elif node.kind == 'terminal_pattern':
        excerpt = {'kind': payload.get('kind'), 'command_count': payload.get('command_count'), 'error_count': payload.get('error_count'), 'confidence': payload.get('confidence')}
        ...
        excerpt = {'claim_type': payload.get('claim_type'), 'artifact_name': payload.get('artifact_name'), 'confidence': payload.get('confidence')}
    elif node.kind in _SIGNAL_KINDS:
        excerpt = {k: payload.get(k) for k in ('score', 'kind', 'metric', 'magnitude', 'z') if k in payload}
    return TimelineRow(when=node.start, project=node.project, kind=node.kind, source=node.source, summary=node.summary, node_id=node.id, payload_excerpt=excerpt)
...
def _citation_cell(row: TimelineRow) -> str:
    ...
    if row.kind == 'commit':
        ...
        if isinstance(refs, dict) and (refs.get('prs') or refs.get('issues')):
            ref_str = ','.join([f'pr#{n}' for n in refs.get('prs', [])] + [f'issue#{n}' for n in refs.get('issues', [])])
...
__all__ = ['CurrentStateTimeline', 'DaySection', 'TimelineRow', 'build_current_state_timeline', 'render_current_state_timeline']
