...
TOOL_COMMAND_TOKENS: dict[str, tuple[str, ...]] = {'Bash': ('pytest', 'cargo', 'just', 'npm', 'git', 'make', 'nix', 'ruff', 'mypy'), 'Edit': (), 'Read': (), 'Write': ()}
    ...
    for (day, project), by_source in grouped.items():
        ...
        for i, sourceA in enumerate(sources):
            for sourceB in sources[i + 1:]:
                ...
                edges.append(EvidenceEdge(nodeA.id, nodeB.id, 'same_project_day', f'{project} on {day} ({sourceA}↔{sourceB})', 0.4))
    return tuple(edges)
...
def temporal_overlap_edges(nodes: Sequence[EvidenceNode]) -> tuple[EvidenceEdge, ...]:
    ...
    for node in nodes:
        if node.project and node.start is not None and (node.end is not None) and (node.end >= node.start):
...
def load_symbol_changes_index() -> dict[str, list[dict[str, Any]]]:
    ...
    try:
        ...
        raise FileNotFoundError(f'active symbol-change product is missing: {path}') from exc
...
def extract_overlap_sources_from_nodes(nodes: Sequence[EvidenceNode]) -> 'tuple[list[Any], list[Any], list[dict[str, Any]]]':
...
def overlap_edges_via_substrate(nodes: Sequence[EvidenceNode], *, refresh_id: str) -> tuple[EvidenceEdge, ...]:
    ...
    from lynchpin.substrate.graph import compute_file_overlap_edges, compute_symbol_overlap_edges
    ...
    promote_ai_work_events(mem, refresh_id=refresh_id, events=deduped_events, project_resolver=_project_resolver)
    if symbol_rows:
        promote_symbol_changes(mem, refresh_id=refresh_id, rows=symbol_rows)
    edges.extend(compute_file_overlap_edges(mem, we_refresh_id=refresh_id, commit_refresh_id=refresh_id))
    edges.extend(compute_symbol_overlap_edges(mem, we_refresh_id=refresh_id, commit_refresh_id=refresh_id))
    mem.close()
    return tuple(edges)
...
def polylogue_work_event_tool_overlap_edges(nodes: Sequence[EvidenceNode], *, max_gap_min: float=60.0) -> tuple[EvidenceEdge, ...]:
...
def temporal_proximity_edges(nodes: Sequence[EvidenceNode], *, max_gap_min: int=90) -> tuple[EvidenceEdge, ...]:
    ...
    for group in grouped.values():
        ...
        for idx, left in enumerate(timed):
            ...
            for right in timed[idx + 1:]:
                ...
                edges.append(EvidenceEdge(left.id, right.id, 'temporal_proximity', f'{left.source} within {gap_min}m of {right.source}', proximity_weight(gap_min)))
    return tuple(edges)
...
def proximity_weight(gap_min: int) -> float:
    ...
    if gap_min <= 60:
        return 0.7
    return 0.58
__all__ = ['load_symbol_changes_index', 'mentions_project_edges', 'overlap_edges_via_substrate', 'polylogue_work_event_tool_overlap_edges', 'same_project_day_edges', 'temporal_overlap_edges', 'temporal_proximity_edges']
