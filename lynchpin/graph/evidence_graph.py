...
def _add_spotify(nodes: list[EvidenceNode], *, start: date, end: date, selected: set[str]) -> None:
    ...
    for d, day_streams in by_day.items():
        ...
        top_tracks = Counter((s.track for s in day_streams if hasattr(s, 'track'))).most_common(5)
        minutes = sum(((getattr(s, 'ms_played', 0) or 0) / 60000 for s in day_streams))
        nodes.append(EvidenceNode(id=f'spotify:listening:{d.isoformat()}', kind='listening_session', source='spotify', date=d, project=None, summary=f"{len(day_streams)} tracks, {minutes:.0f}min — top: {', '.join((a for a, _ in top_artists[:3]))}", payload={'track_count': len(day_streams), 'minutes': round(minutes, 1), 'top_artists': [(a, c) for a, c in top_artists], 'top_tracks': [(t, c) for t, c in top_tracks]}, provenance=EvidenceProvenance('spotify', 'local-fast')))
