...
def build_substrate_confidence_matrix(*, readiness: SourceReadinessReport, graph: EvidenceGraph, correlation_rows: Sequence[CorrelatedWorkDay]=()) -> SubstrateConfidenceMatrix:
    ...
    return SubstrateConfidenceMatrix(rows=tuple(rows), overall_tier=overall_tier, overall_summary=overall_summary)
...
def _ordered_layers(readiness: SourceReadinessReport, graph: EvidenceGraph) -> list[str]:
    ...
    canonical = ['git', 'polylogue', 'activitywatch', 'terminal', 'github', 'raw_log', 'browser', 'sleep', 'health', 'analysis']
...
def _kind_quality_for_polylogue(graph: EvidenceGraph) -> ConfidenceCell:
    ...
    high = sum((1 for n in work_events if (n.payload or {}).get('kind_tier') == 'high'))
    medium = sum((1 for n in work_events if (n.payload or {}).get('kind_tier') == 'medium'))
    total = len(work_events)
    high_pct = high / total
    if high_pct >= 0.75:
        return ConfidenceCell(tier='high', detail=f'{high_pct:.0%} high tier ({high}/{total})')
    if (high + medium) / total >= 0.75:
        return ConfidenceCell(tier='medium', detail=f'{high}/{total} high + {medium}/{total} medium')
...
def _cross_source_cell(layer: str, cross_source_index: dict[str, set[date]], *, window_days: int) -> ConfidenceCell:
...
__all__ = ['ConfidenceCell', 'LayerRow', 'SubstrateConfidenceMatrix', 'Tier', 'build_substrate_confidence_matrix', 'render_substrate_confidence_matrix']
