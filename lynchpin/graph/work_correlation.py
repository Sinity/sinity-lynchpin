...
class _MutableCorrelatedWorkDay:
    ...
    # Event-level kind observations only. Session-profile kind labels are too
    # coarse to merge into project/day claims without overstating certainty.
...
class CorrelatedWorkDay:
    ...
    sources: tuple[str, ...]
    # Arc B.1: kind breakdown. Tuple-of-pairs keeps the dataclass hashable;
    # callers can `dict(row.ai_kind_breakdown)` when they need a map. Defaults
    # to empty so tests / existing constructions don't have to supply them.
...
class WorkEvidenceClaim:
    ...
    summary: str
    caveats: tuple[str, ...] = ()
    # Composite edge IDs (source_id->target_id:relation) for the strongest
    # relations. Used by analysis_claim.relation_ids so load_claim_evidence
    # can join back to real evidence_edge rows. Empty for legacy paths.
...
class _RelationSupport:
    ...
    evidence: str
    weight: float
    # The actual evidence_edge that won the dimension. Carries the
    # composite edge key (source_id->target_id:relation) so analysis_claim
    # rows can reference real edges instead of dimension labels.
    edge_id: str = ""


def correlate_work_days(
    *,
    git_facts: Sequence[object] = (),
    github_items: Sequence[GitHubItem | dict[str, object]] = (),
    ai_sessions: Sequence[object] = (),
    raw_log_entries: Sequence[object] = (),
    focus_spans: Sequence[object] = (),
    shell_sessions: Sequence[object] = (),
...
def summarize_work_correlations(rows: Sequence[CorrelatedWorkDay]) -> WorkCorrelationSummary:
    ...
    return WorkCorrelationSummary(
        row_count=len(rows),
        cross_source_row_count=sum(1 for row in rows if row.has_cross_source_support),
        projects=tuple(sorted({row.project for row in rows})),
        source_counts=dict(sorted(source_counts.items())),
        source_pair_counts=dict(sorted(source_pair_counts.items())),
        git_without_ai_or_focus=git_without_ai_or_focus,
        ai_without_git=ai_without_git,
        focus_without_git=focus_without_git,
        terminal_without_git=terminal_without_git,
    )


def work_day_correlations(
    *,
    start: date,
    end: date,
    ...
    if evidence_graph is None:
        evidence_graph = build_evidence_graph(
            ...
            include_github_frontier=include_github_context,
        )
...
def _correlations_from_graph(
    ...
    *,
    ...
    for node in getattr(graph, "nodes", ()):
        project = _normalize_project(getattr(node, "project", None))
        day = getattr(node, "date", None)
        if project is None or day is None or day < start or day > end:
            continue
        bucket = row(day, project)
        ...
        if kind == "commit":
            ...
            if commit:
                bucket.commit_shas.add(commit)
            summary = str(getattr(node, "summary", "") or "")
            if summary:
                bucket.commit_subjects.append(summary)
            refs = payload.get("github_refs") if isinstance(payload, dict) else None
            if isinstance(refs, dict):
                for ref_kind, singular in (("prs", "pr"), ("issues", "issue")):
                    for number in refs.get(ref_kind, ()) or ():
                        bucket.github_refs.add(f"{singular}#{number}")
        elif kind in {"github_issue", "github_pr", "github_ref"}:
            bucket.sources.add("github_ref" if kind == "github_ref" else "github")
            ref_kind = "pr" if kind == "github_pr" or payload.get("kind") == "pr" else "issue"
            number = int(payload.get("number") or 0)
            if number:
                bucket.github_refs.add(f"{ref_kind}#{number}")
            lifecycle = str(payload.get("lifecycle") or "referenced")
            if lifecycle and lifecycle != "referenced":
                bucket.github_lifecycle_refs[lifecycle].add(f"{ref_kind}#{number}")
        elif kind == "ai_session":
            bucket.sources.add("polylogue")
            conversation_id = str(payload.get("conversation_id") or "").strip()
            if conversation_id:
                bucket.ai_conversation_ids.add(conversation_id)
        elif kind == "ai_work_event":
            # Per-event kind is the preferred signal (Arc A + Arc K).
            event_kind = payload.get("kind")
            tier_raw = str(payload.get("kind_tier") or "medium")
            if event_kind:
                from .work_event_kind import tier_weight
                # Defensive cast: Arc K only emits high/medium/low, but
                # work_event_kind isn't always Arc K's output.
                tier = tier_raw if tier_raw in ("high", "medium", "low") else "medium"
                bucket.ai_event_kind_breakdown[event_kind] += 1
                bucket.ai_event_kind_weighted[event_kind] = (
                    bucket.ai_event_kind_weighted.get(event_kind, 0.0)
                    + tier_weight(tier)  # type: ignore[arg-type]
                )
        elif kind == "raw_log":
            bucket.sources.add("raw_log")
            ...
            bucket.shell_command_count += int(payload.get("command_count") or 0)
    return tuple(_freeze(row) for row in sorted(rows.values(), key=lambda item: (item.date, item.project)))


def render_work_day_correlations(rows: Sequence[CorrelatedWorkDay]) -> str:
    lines = [
        "| Date | Project | Sources | Commits | GitHub | AI Sessions | Raw Log | Focus h | Shell cmds |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in rows:
        github = _format_refs(row.github_refs)
        lines.append(
            "| {date} | {project} | {sources} | {commits} | {github} | {ai} | {raw_log} | {focus:.2f} | {shell} |".format(
                date=row.date.isoformat(),
                project=row.project,
                sources=", ".join(row.sources),
                commits=row.commit_count,
                github=github,
                ai=row.ai_session_count,
                raw_log=row.raw_log_count,
                focus=row.focus_minutes / 60.0,
                shell=row.shell_command_count,
            )
        )
    return "\n".join(lines)
...
def render_work_correlation_summary(summary: WorkCorrelationSummary) -> str:
    ...
    lines = [
        f"- Project/day rows: {summary.row_count}",
        f"- Cross-source rows: {summary.cross_source_row_count}",
        f"- Projects: {', '.join(summary.projects)}",
        f"- Source counts: {_format_counts(summary.source_counts)}",
        f"- Source pair counts: {_format_counts(summary.source_pair_counts)}",
        f"- Git without AI or focus: {summary.git_without_ai_or_focus}",
        f"- AI without git: {summary.ai_without_git}",
        f"- Focus without git: {summary.focus_without_git}",
        f"- Terminal without git: {summary.terminal_without_git}",
    ]
    return "\n".join(lines)


def strongest_work_correlations(
    ...
    return tuple(
        sorted(
            ...
            key=lambda row: (
                row.source_count,
                row.commit_count,
                row.ai_session_count,
                len(row.github_refs),
                row.raw_log_count,
                row.focus_minutes,
                row.shell_command_count,
            ),
            reverse=True,
        )[:limit]
    )
...
def supported_work_claims(
    rows: Sequence[CorrelatedWorkDay],
    *,
    graph: object | None = None,
    limit: int = 12,
) -> tuple[WorkEvidenceClaim, ...]:
...
def render_supported_work_claims(claims: Sequence[WorkEvidenceClaim]) -> str:
    lines = [
        "| Date | Project | Support | Score | Sources | Relations | Claim | Caveats |",
        "|---|---:|---:|---:|---|---|---|---|",
    ]
    for claim in claims:
        lines.append(
            "| {date} | {project} | {support} | {score:.2f} | {sources} | {relations} | {summary} | {caveats} |".format(
                date=claim.date.isoformat(),
                project=claim.project,
                support=claim.support_level,
                score=claim.score,
                sources=", ".join(claim.sources),
                relations="<br>".join(_markdown_cell(value) for value in claim.strongest_relations),
                summary=_markdown_cell(claim.summary),
                caveats="<br>".join(_markdown_cell(value) for value in claim.caveats),
            )
        )
    if not claims:
...
def dataset_correlations(
    ...
    *,
    ...
    for relation in evidence_relations(graph, limit=10_000):
        ...
        example = (
            f"{relation.date.isoformat()} {project}: {relation.relation} - "
            f"{relation.evidence} ({relation.source_summary} ↔ {relation.target_summary})"
        )
...
def render_dataset_correlations(rows: Sequence[DatasetCorrelation]) -> str:
    ...
    lines = [
        "| Sources | Score | Relations | Projects | Dates | Examples |",
        "|---|---:|---|---|---|---|",
    ]
    if not rows:
    ...
    for row in rows:
        lines.append(
            "| {sources} | {score:.2f} | {relations} | {projects} | {dates} | {examples} |".format(
                sources=" + ".join(row.sources),
                score=row.score,
                relations=_format_counts(row.relation_counts),
                projects=", ".join(row.projects),
                dates=", ".join(day.isoformat() for day in row.dates[:5]),
                examples="<br>".join(_markdown_cell(example) for example in row.examples),
            )
        )
    return "\n".join(lines)
...
def _freeze(row: _MutableCorrelatedWorkDay) -> CorrelatedWorkDay:
    return CorrelatedWorkDay(
        date=row.date,
        project=row.project,
        commit_count=len({sha for sha in row.commit_shas if sha}),
        commit_shas=tuple(sorted(sha for sha in row.commit_shas if sha)),
        commit_subjects=tuple(row.commit_subjects),
        github_refs=tuple(sorted(row.github_refs)),
        github_lifecycles={
            lifecycle: len(refs)
            for lifecycle, refs in sorted(row.github_lifecycle_refs.items())
        },
        ai_session_count=len({cid for cid in row.ai_conversation_ids if cid}),
        ai_conversation_ids=tuple(sorted(cid for cid in row.ai_conversation_ids if cid)),
        ai_kind_breakdown=tuple(
            sorted(row.ai_event_kind_breakdown.items(), key=lambda kv: (-kv[1], kv[0]))
        ),
        ai_kind_weighted=tuple(
            (kind, round(weight, 3))
            for kind, weight in sorted(
                row.ai_event_kind_weighted.items(),
                key=lambda kv: (-kv[1], kv[0]),
            )
        ),
        raw_log_count=len(row.raw_log_refs),
        raw_log_refs=tuple(sorted(row.raw_log_refs)),
        focus_minutes=round(row.focus_minutes, 2),
        shell_minutes=round(row.shell_minutes, 2),
...
def _freeze_dataset_correlation(bucket: _MutableDatasetCorrelation) -> DatasetCorrelation:
    ...
    return DatasetCorrelation(
        ...
        projects=tuple(sorted(bucket.projects)),
        dates=tuple(sorted(bucket.dates, reverse=True)),
        examples=tuple(
            bucket.context_examples[key]
            for key, _weight in sorted(
                bucket.context_weights.items(),
...
def _relations_by_project_day(graph: object | None) -> dict[tuple[date, str], tuple[_RelationSupport, ...]]:
    ...
    for relation in evidence_relations(graph, limit=10_000):
        ...
        if relation.source_source == "analysis" or relation.target_source == "analysis":
            continue
        dimension = _relation_dimension(
            relation.relation,
            relation.source_source,
            relation.target_source,
        )
        edge_id = f"{relation.source_node_id}->{relation.target_node_id}:{relation.relation}"
        support = _RelationSupport(
            dimension=dimension, evidence=relation.evidence,
            weight=relation.weight, edge_id=edge_id,
        )
    ...
    return {
        key: tuple(sorted(value.values(), key=lambda item: (item.weight, item.dimension), reverse=True))
...
def _work_claim(row: CorrelatedWorkDay, relations: Sequence[_RelationSupport]) -> WorkEvidenceClaim:
    ...
    top = relations[:3]
    return WorkEvidenceClaim(
        date=row.date,
        project=row.project,
        support_level=_support_level(row, relation_count),
        score=_claim_score(row, relation_count),
        sources=row.sources,
        relation_count=relation_count,
        strongest_relations=tuple(f"{relation.dimension}: {relation.evidence}" for relation in top),
        summary=_claim_summary(row, relation_count),
        caveats=_claim_caveats(row, relation_count),
        strongest_edge_ids=tuple(rel.edge_id for rel in top if rel.edge_id),
    )
...
def _claim_score(row: CorrelatedWorkDay, relation_count: int) -> float:
    return round(
        row.source_count
        + min(relation_count, 6) * 0.35
        + min(row.commit_count, 5) * 0.12
        + min(row.ai_session_count, 3) * 0.2
        + min(row.focus_minutes / 60.0, 6.0) * 0.08
        + min(row.shell_command_count, 12) * 0.04,
        3,
    )
...
def _claim_caveats(row: CorrelatedWorkDay, relation_count: int) -> tuple[str, ...]:
    ...
    if row.github_refs:
        caveats.append("GitHub refs may be commit-message references unless github_context lifecycle evidence is present")
...
def _github_item_from_context(item: object) -> GitHubItem:
    ...
    for raw_comment in item.get("comments") or []:
        ...
        comments.append(GitHubComment(
            author=GitHubActor(raw_author.get("login") if isinstance(raw_author, dict) else None),
