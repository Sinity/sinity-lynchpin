...
def project_day_correlations(
    ...
    """Query the project_day_correlation view.

    Uses ``lynchpin.substrate.derived.load_project_day_correlations``.

    Parameters:
        refresh_id:       filter to a specific evidence-graph build.
        start:            ISO date string (inclusive lower bound on date column).
        end:              ISO date string (inclusive upper bound on date column).
        projects:         list of project names to include; None = all.
        min_source_count: only return rows backed by at least N distinct sources.
                          Use 2 for cross-source-verified project-days.

    Returns list of dicts with keys: project, date, refresh_id, commit_count,
    ai_session_count, ai_work_event_count, github_item_count, focus_count,
    terminal_count, raw_log_count, commit_shas,
    conversation_ids, github_node_ids, focus_minutes, shell_minutes,
    source_count.
    """
...
def file_overlap_edges(
    ...
    with connect(path) as conn:
        edges = compute_file_overlap_edges(
            ...
            commit_refresh_id=commit_refresh_id,
        )
    return [
...
def context_pack_diff(
    refresh_a: str | None = None,
    refresh_b: str | None = None,
) -> dict[str, Any]:
    ...
    with connect(substrate_path(), read_only=True) as conn:
        ...
        for table in tables:
            try:
                a_row = conn.execute(
                    f"SELECT COUNT(*) FROM {table} WHERE refresh_id = ?",
                    [refresh_a],
                ).fetchone()
                b_row = conn.execute(
                    f"SELECT COUNT(*) FROM {table} WHERE refresh_id = ?",
                ...
                diffs[table] = {
                    ...
                    "b": -1,
                    "delta": 0,
                    "error": f"table {table} may not have refresh_id column",
                }

        # Project-day changes (top 20 by commit delta)
        ...
        try:
            ...
            diffs["source_status_diffs"] = [
                {"source": r[0], "status_a": r[1], "status_b": r[2]} for r in st_rows
            ]
        except Exception:
            pass
...
def frontier_slo(
    ...
    materialization = (
        ensure_substrate_materialized_for_read(caller="frontier_slo")
        if refresh_id is None
        else pinned_materialization_for_read(caller="frontier_slo", refresh_id=refresh_id)
    )
    with connect(substrate_path(), read_only=True) as conn:
        if refresh_id is None:
    ...
    return {
        ...
        "overall": {
            "p50_merge_m": overall[0] if overall else None,
            "p75_merge_m": overall[1] if overall else None,
            "p95_merge_m": overall[2] if overall else None,
            "avg_rounds": overall[3] if overall else None,
        },
        "friction_breakdown": [
            {"project": r[0], "signal": r[1], "count": r[2]} for r in friction
        ],
    }


def project_pair_signals(
    project_a: str | None = None,
    project_b: str | None = None,
    refresh_id: str | None = None,
    limit: int = 50,
) -> list[dict[str, Any]]:
    """Cross-project relationship breakdown by signal kind.

    Returns the full signal decomposition: shared AI sessions, shared AI work
    events, and shared raw-log mentions.
    Each signal carries a weight per
    ``lynchpin/graph/project_relationships.py:54-60``; total weight is
    the per-signal weighted sum.

    Note: shared_commits (cross-repo PR/issue refs) was REMOVED because
    bare ``#N`` numbers collide across repos with no real coordination
    (e.g. sinex#542 and polylogue#542 are unrelated). Genuine cross-repo
    refs need owner/repo#N which the upstream extractor doesn't preserve.

    Parameters:
        project_a, project_b:
            Both given → return the single pair (in either order).
            Either omitted → return all pairs ranked by total_weight.
        refresh_id:
            Substrate snapshot. Defaults to latest evidence_graph build.
        limit:
            Cap on rows when scanning all pairs.
...
def walk_evidence(
    ...
    materialization = (
        ensure_substrate_materialized_for_read(caller="walk_evidence")
        if refresh_id is None
        else pinned_materialization_for_read(caller="walk_evidence", refresh_id=refresh_id)
    )
    path = substrate_path()
    with connect(path, read_only=True) as conn:
        if refresh_id is None:
            ...
            if refresh_id is None:
                return {
                    "start_id": start_id, "direction": direction,
                    "edge_kinds": edge_kinds, "max_depth": max_depth,
    ...
    result = _walk(
        graph, start_id,
        edge_kinds=edge_kinds,  # type: ignore[arg-type]
    ...
    return {
        ...
        "edge_kinds": list(result.edge_kinds) if result.edge_kinds else None,
        ...
        "nodes": [
            {
                ...
                "date": _json_safe(step.node.date),
                "project": step.node.project, "summary": step.node.summary,
                "depth": step.depth,
                "parent_edge_source": step.parent_edge.source_id if step.parent_edge else None,
                "parent_edge_target": step.parent_edge.target_id if step.parent_edge else None,
                "parent_edge_relation": step.parent_edge.relation if step.parent_edge else None,
                "direction_followed": step.direction_followed,
            }
...
def url_crossref(
    ...
    min_sources: int = 2,
    sources: str = "reddit,irc,wykop,raindrop",
    limit: int = 100,
) -> list[dict[str, Any]]:
    ...
    return [
        {
            ...
            "sample_snippets": list(a.sample_snippets),
        }
        for a in filtered
    ]
