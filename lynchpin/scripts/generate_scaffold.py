...
def _summarize_ai(days: list, events: list | None = None) -> dict[str, Any]:
    """Compact Polylogue rollup for narrative period scaffolds."""
    if not days and not events:
        return {}
...
class BatchSources:
    ...
    def __init__(self, start: date, end: date):
        ...
        self.sleep_stages = _load("Sleep stages", sleep_stages, start=start - timedelta(days=1), end=end)
        self.sleep_architecture = _load("Sleep architecture", sleep_architecture, start=start - timedelta(days=1), end=end)
        # Health
        ...
        self.ecg = _load("ECG", ecg_measurements, start=start, end=end)
        # Web
...
def generate_day(d: date, output: Path, *, force: bool = False, all_features: list | None = None,
    ...
    if sleep_architecture_day:
        write_json(day_dir / "sleep_architecture.json", sleep_architecture_day)
...
def generate_week(week_key: str, output: Path, *, force: bool = False,
    ...
    write_json(week_dir / "week_metrics.json", {
        ...
        "sleep": _summarize_sleep(sleep, sleep_arch),
        "health": _summarize_health(health),
        "ai": _summarize_ai(poly_summaries, poly_events),
...
def generate_year(year_key: str, output: Path, *, force: bool = False,
    ...
    write_json(y_dir / "year_metrics.json", {
        ...
        "sleep": _summarize_sleep(sleep, sleep_arch),
        "health": _summarize_health(health),
        "ai": _summarize_ai(chat_act, poly_events),
    })
...
def generate_hierarchy(start: date, end: date, output: Path, *, force: bool = False, dry_run: bool = False, skip_empty: bool = False) -> bool:
    ...
    if skip_empty:
        ...
        poly_days = set()
        for p in _safe(_poly_days, start=start, end=end, default=[]):
        ...
        web_days = {w.date for w in _safe(_web_days, start=start, end=end, default=[])}
        msg_days = {m.date for m in _safe(_msg_days, start=start, end=end, default=[])}
        bm_days = {b.date for b in _safe(_bm_days, start=start, end=end, default=[])}
