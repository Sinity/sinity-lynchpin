    ...
    *, start: date, end: date, min_gap_hours: float = 3.0,
    ...
    for gap_start, gap_end, gap_h in gaps:
        ...
        if best_watch is not None:
            ...
            # Build stage breakdown if available
            stages = None
            metrics = getattr(entry, '_raw_metrics', None)
            # Try to get from the entry's data
            if hasattr(entry, 'segments') and entry.segments:
                # We don't have per-stage data on SleepEntry directly,
                # but sleep_metrics may have it from our processing
                pass
