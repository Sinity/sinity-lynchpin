...
from ..ingest.fbmessenger_db import ensure_export_db_compatibility
...
from .validate_common import _count_iter, _exit_on_failures, _latest_takeout_archive, _run_checks, _sample_iter
from .validate_hpi import _select_hpi_modules
...
def lynchpin(
    ...
    def _instrumentation_terminal_sessions() -> Tuple[Optional[int], str]:
        entries, truncated = _sample_iter(terminal_capture.iter_terminal_audit(), sample_limit)
        summary = terminal_capture.summarize_terminal_audit(iter(entries))
    ...
    def _instrumentation_terminal_events() -> Tuple[Optional[int], str]:
        events, truncated = _sample_iter(terminal_capture.iter_terminal_session_events(), sample_limit)
        detail = (
    ...
    def _instrumentation_audio() -> Tuple[Optional[int], str]:
        count, truncated = _count_iter(media_capture.iter_audio_recordings(), sample_limit)
    ...
    def _instrumentation_screen() -> Tuple[Optional[int], str]:
        count, truncated = _count_iter(media_capture.iter_screenshots(), sample_limit)
    ...
    def _takeout_archives() -> Tuple[Optional[int], str]:
        ...
        parts = takeout_archives.expand_takeout_parts(latest)
