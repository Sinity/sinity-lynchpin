...
def _scrape_api_extras(
    *,
    ...
    def dump_json(key: str, endpoint: str, filename: str, *, params: dict[str, Any] | None = None) -> None:
    ...
    dump_json("settings_changephone", "settings/changephone", "wykop_settings_changephone.json")
    ...
    for key, endpoint, filename in [
        ...
        pages = 0
        try:
            for token, payload in _api_iter_cursor_pages(api_client, endpoint, params={}, max_pages=max_pages):
                pages += 1
                data = payload.get("data") if isinstance(payload, dict) else None
                if isinstance(data, list):
                    for item in data:
    ...
    observed_discussions_rows: list[dict[str, Any]] = []
    pages = 0
    try:
        for token, payload in _api_iter_cursor_pages(api_client, "observed/discussions", params={}, max_pages=max_pages):
            pages += 1
            data = payload.get("data") if isinstance(payload, dict) else None
            if isinstance(data, list):
                for item in data:
                    if isinstance(item, dict):
                        observed_discussions_rows.append(
                            {
                                "platform": "wykop",
        ...
        record(
            ...
            {"output": str(observed_discussions_path), "endpoint": "observed/discussions", "ok": False, "status": status, "error": str(e)},
        )
    else:
        _write_jsonl(observed_discussions_path, observed_discussions_rows)
        record(
            "observed_discussions",
            {
                ...
                "endpoint": "observed/discussions",
                "ok": True,
                "items": len(observed_discussions_rows),
                "pages": pages,
            },
        )

    tags_path = user_dir / "wykop_observed_tags.json"
    ...
    record("observed_tags", {"output": str(tags_path), "ok": True, "items": len(observed_tags), "pages": pages})
...
def _parse_entries_page(root: etree._Element, page: int, username: str, kind: str, authored_only: bool) -> list[dict[str, Any]]:
    ...
    for section in root.xpath("//section[starts-with(@id,'comment-') and contains(@class,'entry')]"):
        ...
        if authored_only and author != username:
            continue
...
def scrape(
    ...
    out_dir: Path = typer.Option(
        Path("/realm/data/personal-data/my_external_exports/wykop"),
    ...
    user_agent: str = typer.Option(
        "Mozilla/5.0 (compatible; sinity-analysis/wykop-scraper; +https://wykop.pl)",
