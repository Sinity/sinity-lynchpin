...
def test_machine_episode_detector_merges_multisource_pressure(tmp_path):
    ...
    with connect(db) as conn:
        ...
        conn.execute(
            ...
            INSERT INTO machine_metric_sample (
                ...
                (?, 'host', 'machine.telemetry', 2, 1, 32000, 0, 0, 1, 0, 40, 5, 4, 16, [], 'r1'),
                (?, 'host', 'machine.telemetry', 2, 28, 1800, 16, 2, 75, 2, 86, 98, 2, 16, ['collector.late'], 'r1'),
                (?, 'host', 'machine.telemetry', 2, 31, 1700, 18, 3, 80, 2, 87, 99, 2, 16, [], 'r1')
            """,
            [
    ...
    cpu = next(episode for episode in analysis.episodes if episode.kind == "cpu_saturation")
    assert cpu.sample_count == 2
    assert cpu.started_at.minute == 1
    assert cpu.ended_at.minute == 2
    assert {item.metric for item in cpu.evidence} >= {"load_1m"}
    assert any("collector.late" in caveat for caveat in cpu.caveats)
