...
def test_work_day_correlations_projects_from_evidence_graph():
    ...
    graph = EvidenceGraph(
        ...
        nodes=(
            EvidenceNode(
                ...
                summary="feat: graph (#3)",
                start=when,
                end=when,
                payload={"commit": "abc", "github_refs": {"prs": [3], "issues": []}},
            ),
            EvidenceNode(
                id="polylogue:conv:sinity-lynchpin",
                kind="ai_session",
                source="polylogue",
                date=when.date(),
                project="sinity-lynchpin",
        ...
        edges=(
            EvidenceEdge(
                source_id="git:lynchpin:abc",
                target_id="polylogue:conv:sinity-lynchpin",
                relation="temporal_proximity",
                evidence="git within 0m of polylogue",
                weight=0.82,
            ),
        ),
        caveats=(),
    ...
    # strongest_edge_ids must carry the actual edge composite key
    # (source_id->target_id:relation), not the dimension label. This lets
    # load_claim_evidence join back to evidence_edge rows.
    assert claims[0].strongest_edge_ids == (
        "git:lynchpin:abc->polylogue:conv:sinity-lynchpin:temporal_proximity",
    )
...
def test_build_evidence_graph_links_temporal_proximity_across_sources(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: (
            SimpleNamespace(
                ...
                commit="abc123",
                authored_at=commit_at,
                author="Sinity",
                subject="fix: correlate nearby work",
                lines_added=3,
                lines_deleted=1,
                lines_changed=4,
                files_changed=1,
    ...
    monkeypatch.setattr(
        "lynchpin.graph.evidence_terminal.shell_sessions",
        lambda **kwargs: (
            SimpleNamespace(
                start=shell_at,
                end=datetime(2026, 5, 5, 12, 25, tzinfo=UTC),
                project="sinity-lynchpin",
    ...
    assert any(
        edge.relation == "temporal_proximity" and "within 20m" in edge.evidence
