"""Tests for best_materialized_refresh_id with substrate_source_status awareness.
...
def test_best_materialized_refresh_id_prefers_ok_source_status(monkeypatch, tmp_path) -> None:
    ...
    with connect(substrate_path(), read_only=True) as test_conn:
        result = best_materialized_refresh_id(test_conn, "commit_fact", caller="test.ok_status")
...
def test_latest_materialized_refresh_id_prefers_successful_promotion_run(tmp_path) -> None:
    ...
    conn.execute(
        ...
        ["r-full", "ok", datetime(2026, 5, 24, 10, tzinfo=timezone.utc)],
    )
    conn.execute(
        "INSERT INTO substrate_source_status VALUES (?, ?, ?, ?)",
        ["r-narrow", "machine", "ok", datetime(2026, 5, 25, 10, tzinfo=timezone.utc)],
    )
    conn.close()

    test_conn = duckdb.connect(str(db_path), read_only=True)
    try:
        assert latest_materialized_refresh_id(test_conn, caller="test.latest") == "r-full"
    finally:
        test_conn.close()


def test_ordered_materialized_refresh_ids_prefers_promotion_runs(tmp_path) -> None:
    import duckdb
    ...
    conn.execute(
        "INSERT INTO substrate_promotion_run VALUES (?, ?, ?)",
        ["r-old", "ok", datetime(2026, 5, 24, 10, tzinfo=timezone.utc)],
    )
    conn.execute(
        "INSERT INTO substrate_promotion_run VALUES (?, ?, ?)",
        ["r-new", "ok", datetime(2026, 5, 25, 10, tzinfo=timezone.utc)],
    )
    conn.execute(
        "INSERT INTO substrate_source_status VALUES (?, ?, ?, ?)",
        ["r-narrow", "machine", "ok", datetime(2026, 5, 26, 10, tzinfo=timezone.utc)],
    )
    conn.close()

    test_conn = duckdb.connect(str(db_path), read_only=True)
    try:
        assert ordered_materialized_refresh_ids(test_conn, caller="test.ordered") == ["r-old", "r-new"]
    finally:
        test_conn.close()
...
def test_snapshot_helpers_do_not_fallback_when_promotion_runs_are_failed_only(
    ...
    conn.execute(
        "INSERT INTO substrate_promotion_run VALUES (?, ?, ?)",
        ["r-failed", "error", datetime(2026, 5, 24, 10, tzinfo=timezone.utc)],
    )
    conn.execute(
        "INSERT INTO substrate_source_status VALUES (?, ?, ?, ?)",
        ["r-failed", "commits", "error", datetime(2026, 5, 24, 10, tzinfo=timezone.utc)],
    )
    conn.close()
...
def test_best_materialized_refresh_id_maps_work_observation_tables_to_source_status(tmp_path) -> None:
    import duckdb
    ...
    conn.execute(
        "INSERT INTO work_observation VALUES (?, ?, ?)",
        ["older", "r-old", datetime(2026, 5, 24, 10, tzinfo=timezone.utc)],
    )
    conn.execute(
        "INSERT INTO work_observation VALUES (?, ?, ?)",
        ["newer", "r-new", datetime(2026, 5, 25, 10, tzinfo=timezone.utc)],
    )
    conn.execute(
        "INSERT INTO substrate_source_status VALUES (?, ?, ?, ?)",
        ["r-old", "work_observations", "ok", datetime(2026, 5, 24, 11, tzinfo=timezone.utc)],
    )
    conn.close()

    test_conn = duckdb.connect(str(db_path), read_only=True)
    try:
        assert best_materialized_refresh_id(test_conn, "work_observation", caller="test.work_observation") == "r-old"
    finally:
        test_conn.close()
...
def test_best_materialized_refresh_id_maps_machine_experiment_table_to_source_status(tmp_path) -> None:
    ...
    try:
        assert best_materialized_refresh_id(
            ...
            caller="test.machine_experiment_run",
        ) == "r-old"
    finally:
        test_conn.close()


def test_best_materialized_refresh_id_falls_back_to_latest_materialized_at(
    ...
    with connect(substrate_path(), read_only=True) as test_conn:
        result = best_materialized_refresh_id(test_conn, "commit_fact", caller="test.fallback")
    # Should pick id2 because it has the latest materialized_at
    assert result == id2


def test_best_materialized_refresh_id_does_not_fallback_when_source_status_is_non_ok(
    tmp_path,
) -> None:
    import duckdb
    ...
    conn.execute(
        "INSERT INTO commit_fact VALUES (?, ?, ?)",
        ["sha-error", "r-error", datetime(2026, 5, 25, 0, tzinfo=timezone.utc)],
    )
    conn.execute(
        "INSERT INTO substrate_source_status VALUES (?, ?, ?, ?)",
        ["r-error", "commits", "error", datetime(2026, 5, 25, 1, tzinfo=timezone.utc)],
    )
    conn.close()

    test_conn = duckdb.connect(str(db_path), read_only=True)
    try:
        assert best_materialized_refresh_id(test_conn, "commit_fact", caller="test.non_ok") is None
    finally:
        test_conn.close()


def test_best_materialized_refresh_id_does_not_fallback_when_ok_status_has_no_rows(
    tmp_path,
) -> None:
    import duckdb
    ...
    conn.execute(
        "INSERT INTO commit_fact VALUES (?, ?, ?)",
        ["sha-unstatused", "r-unstatused", datetime(2026, 5, 25, 0, tzinfo=timezone.utc)],
    )
    conn.execute(
        "INSERT INTO substrate_source_status VALUES (?, ?, ?, ?)",
        ["r-empty", "commits", "ok", datetime(2026, 5, 25, 1, tzinfo=timezone.utc)],
    )
    conn.close()

    test_conn = duckdb.connect(str(db_path), read_only=True)
    try:
        assert best_materialized_refresh_id(test_conn, "commit_fact", caller="test.ok_empty") is None
    finally:
        test_conn.close()


def test_best_materialized_refresh_id_returns_none_for_empty_table(monkeypatch, tmp_path) -> None:
    ...
    with connect(substrate_path(), read_only=True) as conn:
        result = best_materialized_refresh_id(conn, "commit_fact", caller="test.empty")
...
def test_fallback_prefers_high_coverage_over_recent_when_no_status(
    ...
    Regression for the bug where activity_content_day had a recent narrow DAG
    materialization with 6 rows and an older wider current-state materialization
    with 404 rows; the "latest materialized_at" fallback picked the 6-row
    materialization and downstream queries silently returned mostly-empty
    results.
    """
    import duckdb
    ...
    # Older materialization: 5 rows (comprehensive)
    for i in range(5):
        conn.execute(
            "INSERT INTO commit_fact VALUES (?, ?, ?)",
    ...
    # Newer materialization: 1 row (narrow window)
    conn.execute(
        "INSERT INTO commit_fact VALUES (?, ?, ?)",
    ...
    from lynchpin.substrate.connection import connect, substrate_path
    with connect(substrate_path(), read_only=True) as test_conn:
        result = best_materialized_refresh_id(test_conn, "commit_fact", caller="test.high_coverage")
    assert result == older_comprehensive, (
        f"expected the 5-row materialization to win over the 1-row recent materialization "
