...
def test_list_evidence_graph_builds_with_one(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Header estimates do not substitute for measuring actual fixture rows.
    assert result[0]["graph_integrity"]["orphaned_edges"] == 0
    assert result[0]["graph_integrity"]["total_edges"] == 0
    assert result[0]["graph_integrity"]["measured"] is True
...
def test_load_evidence_graph_summary_reports_materialization_on_missing_build(
    ...
    assert result["graph_integrity"]["orphan_ratio"] is None
