...
def test_respiratory_rate_preserves_upper_limit(monkeypatch):
    def fake_load(filename):
        assert filename == "health_respiratory.jsonl"
        yield {
            "start_time": "2025-05-21T05:53:00+02:00",
            "avg_rate": 12.0,
            "lower_limit": 10.0,
            "upper_limit": 20.0,
        }

    monkeypatch.setattr("lynchpin.sources.health._load_jsonl", fake_load)
...
def test_ecg_measurements_loader(monkeypatch):
    def fake_load(filename):
        assert filename == "health_ecg.jsonl"
        yield {
            "start_time": "2023-03-22T00:11:47+01:00",
            "end_time": "2023-03-22T00:12:17+01:00",
            "mean_heart_rate": 94.0,
            "sample_count": 15000,
    ...
    assert result[0].mean_heart_rate == 94.0
    assert result[0].sample_frequency == 500
