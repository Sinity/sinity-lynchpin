...
def _insert_node(
    ...
    *,
    ...
    conn.execute(
        ...
        """,
        [refresh_id, node_id, kind, source, date_val, project,
         f"{kind} summary", payload_json, "[]"],
    )
...
def _insert_build(conn, *, refresh_id: str) -> None:
    conn.execute(
        ...
        """,
        [refresh_id, date(2026, 5, 1), date(2026, 5, 7),
         "local-fast", [], 0, 0, "[]", _dt(2026, 5, 7)],
    )
...
class TestProjectDayCorrelation:
    ...
    def test_counts_by_kind(self, tmp_path: Path) -> None:
        ...
        with connect(db) as conn:
            ...
            day = date(2026, 5, 3)
            _insert_node(conn, refresh_id="r1", node_id="c1", kind="commit",
                         source="git", date_val=day, project="lynchpin")
            _insert_node(conn, refresh_id="r1", node_id="c2", kind="commit",
                         source="git", date_val=day, project="lynchpin")
            _insert_node(conn, refresh_id="r1", node_id="ai1", kind="ai_work_event",
                         source="polylogue", date_val=day, project="lynchpin")
            _insert_node(conn, refresh_id="r1", node_id="gh1", kind="github_issue",
                         source="github", date_val=day, project="lynchpin")
            _insert_node(conn, refresh_id="r1", node_id="t1", kind="terminal_session",
                         source="terminal", date_val=day, project="lynchpin")
    ...
    def test_aggregates_commit_shas(self, tmp_path: Path) -> None:
        ...
        with connect(db) as conn:
            ...
            day = date(2026, 5, 4)
            _insert_node(conn, refresh_id="r1", node_id="c1", kind="commit",
                         source="git", date_val=day, project="sinex",
                         payload={"commit": "abc123"})
            _insert_node(conn, refresh_id="r1", node_id="c2", kind="commit",
                         source="git", date_val=day, project="sinex",
                         payload={"commit": "def456"})

            rows = load_project_day_correlations(conn, refresh_id="r1", projects=("sinex",))
    ...
    def test_filters_null_project(self, tmp_path: Path) -> None:
        ...
        with connect(db) as conn:
            ...
            day = date(2026, 5, 5)
            _insert_node(conn, refresh_id="r1", node_id="c1", kind="commit",
                         source="git", date_val=day, project=None)  # NULL project
            _insert_node(conn, refresh_id="r1", node_id="c2", kind="commit",
                         source="git", date_val=day, project="sinex")
    ...
    def test_min_source_count_filter(self, tmp_path: Path) -> None:
        ...
        with connect(db) as conn:
            ...
            _insert_node(conn, refresh_id="r1", node_id="c2", kind="commit",
                         source="git", date_val=day, project="sinex")
            _insert_node(conn, refresh_id="r1", node_id="ai1", kind="ai_work_event",
                         source="polylogue", date_val=day, project="sinex")

            rows = load_project_day_correlations(conn, refresh_id="r1", min_source_count=2)
    ...
    def test_focus_minutes_aggregation(self, tmp_path: Path) -> None:
        ...
        with connect(db) as conn:
            ...
            day = date(2026, 5, 7)
            _insert_node(conn, refresh_id="r1", node_id="f1", kind="focus_day",
                         source="activitywatch", date_val=day, project="lynchpin",
                         payload={"duration_s": 3600.0})  # 60 minutes
            _insert_node(conn, refresh_id="r1", node_id="f2", kind="focus_day",
                         source="activitywatch", date_val=day, project="lynchpin",
                         payload={"duration_s": 1800.0})  # 30 minutes
    ...
    def test_date_range_filter(self, tmp_path: Path) -> None:
        ...
        with connect(db) as conn:
            ...
            for d_val, proj in [
                ...
                _insert_node(conn, refresh_id="r1", node_id=f"c_{proj}", kind="commit",
                             source="git", date_val=d_val, project=proj)

            rows = load_project_day_correlations(
                conn, refresh_id="r1",
                start=date(2026, 5, 2), end=date(2026, 5, 5),
            )
...
class TestIssueClosureChainWalk:
    ...
    def test_simple_two_hop(self, tmp_path: Path) -> None:
        ...
        with connect(db) as conn:
            ...
            day = date(2026, 5, 1)
            _insert_node(conn, refresh_id="r1", node_id="issue:1", kind="github_issue",
                         source="github", date_val=day, project="lynchpin",
                         payload={"number": "42"})
            _insert_node(conn, refresh_id="r1", node_id="pr:99", kind="github_pr",
                         source="github", date_val=day, project="lynchpin")
            _insert_node(conn, refresh_id="r1", node_id="commit:abc", kind="commit",
                         source="git", date_val=day, project="lynchpin")

            _insert_edge(conn, refresh_id="r1", source_id="issue:1",
                         target_id="pr:99", relation="references")
            _insert_edge(conn, refresh_id="r1", source_id="pr:99",
                         target_id="commit:abc", relation="references")
    ...
    def test_orphaned_issue(self, tmp_path: Path) -> None:
        ...
        with connect(db) as conn:
            ...
            _insert_node(conn, refresh_id="r1", node_id="issue:7", kind="github_issue",
                         source="github", date_val=date(2026, 5, 2), project="sinex",
                         payload={"number": "7"})
    ...
    def test_depth_cap_prevents_infinite_loop(self, tmp_path: Path) -> None:
        ...
        with connect(db) as conn:
            ...
            day = date(2026, 5, 3)
            _insert_node(conn, refresh_id="r1", node_id="issue:99", kind="github_issue",
                         source="github", date_val=day, project="lynchpin",
                         payload={"number": "99"})
            _insert_node(conn, refresh_id="r1", node_id="nodeA", kind="github_pr",
                         source="github", date_val=day, project="lynchpin")
            _insert_node(conn, refresh_id="r1", node_id="nodeB", kind="commit",
                         source="git", date_val=day, project="lynchpin")

            # Create a cycle: issue:99 → nodeA → nodeB → issue:99
            _insert_edge(conn, refresh_id="r1", source_id="issue:99",
                         target_id="nodeA", relation="references")
            _insert_edge(conn, refresh_id="r1", source_id="nodeA",
                         target_id="nodeB", relation="references")
            _insert_edge(conn, refresh_id="r1", source_id="nodeB",
                         target_id="issue:99", relation="references")
    ...
    def test_filter_by_project(self, tmp_path: Path) -> None:
        ...
        with connect(db) as conn:
            ...
            day = date(2026, 5, 4)
            _insert_node(conn, refresh_id="r1", node_id="issue:lp1", kind="github_issue",
                         source="github", date_val=day, project="lynchpin",
                         payload={"number": "1"})
            _insert_node(conn, refresh_id="r1", node_id="issue:sx1", kind="github_issue",
                         source="github", date_val=day, project="sinex",
                         payload={"number": "1"})

            rows = load_issue_closure_chain_walks(conn, refresh_id="r1", project="lynchpin")
    ...
    def test_min_chain_depth_filter(self, tmp_path: Path) -> None:
        ...
        with connect(db) as conn:
            ...
            day = date(2026, 5, 5)
            # Orphaned (depth=0)
            _insert_node(conn, refresh_id="r1", node_id="issue:orphan", kind="github_issue",
                         source="github", date_val=day, project="lynchpin",
                         payload={"number": "0"})
            # Has a reference (depth=1)
            _insert_node(conn, refresh_id="r1", node_id="issue:linked", kind="github_issue",
                         source="github", date_val=day, project="lynchpin",
                         payload={"number": "10"})
            _insert_node(conn, refresh_id="r1", node_id="pr:linked", kind="github_pr",
                         source="github", date_val=day, project="lynchpin")
            _insert_edge(conn, refresh_id="r1", source_id="issue:linked",
                         target_id="pr:linked", relation="references")

            rows = load_issue_closure_chain_walks(conn, refresh_id="r1", min_chain_depth=1)
