...
def test_session_profiles_derive_from_base_tables_when_products_empty(monkeypatch, tmp_path):
    ...
    assert profiles[0].conversation_id == "codex:1"
    assert profiles[0].canonical_session_date == date(2026, 4, 22)
    assert profiles[0].message_count == 7
    assert profiles[0].tool_use_count == 2
...
def test_session_profiles_canonicalize_base_table_project_fragments(monkeypatch, tmp_path):
    ...
    with sqlite3.connect(db) as conn:
        conn.execute(
            ...
            (
                '{"working_directories":['
                '"/tmp/conversations-001-37",'
                '"/tmp/polylogue-rebuild",'
                '"/realm/project/_inactive/codex",'
                '"/realm/project/sinex-target-vision"'
                ']}',
                "codex:1",
            ),
        )
    ...
    assert profiles[0].work_event_projects == ("polylogue", "sinex-target-vision")


def test_day_summaries_derive_from_base_profiles_when_products_empty(monkeypatch, tmp_path):
    db = tmp_path / "polylogue.db"
    _write_base_polylogue_db(db)
    monkeypatch.setattr(polylogue, "_default_polylogue_db_path", lambda: db)
    monkeypatch.setattr(polylogue, "_cached_day_summaries", None)

    summaries = polylogue.day_session_summaries()

    assert len(summaries) == 1
    assert summaries[0].date == date(2026, 4, 22)
    assert summaries[0].session_count == 1
    assert summaries[0].total_messages == 7
    assert summaries[0].repos_active == ("sinity-lynchpin",)
